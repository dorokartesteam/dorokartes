...
const payload=path.join(root,"dorokartes-admin-v3-4-payload");
if(!fs.existsSync(payload)){console.error("Extract ZIP at D:\\dorokartes first.");process.exit(1)}

const stamp=new Date().toISOString().replace(/[:.]/g,"-");
const backup=path.join(path.dirname(root),`dorokartes-admin-backup-v3-4-${stamp}`);
...
console.log("Admin v3.4 Launch Readiness installed.");
