...
const VERSION = "verification-enrichment-v18" as const;
...
const CONCURRENCY = Math.max(
  1,
  Math.min(12, Number(process.env.VERIFICATION_V18_CONCURRENCY || "6")),
);
const FETCH_TIMEOUT_MS = Math.max(
  5_000,
  Math.min(30_000, Number(process.env.VERIFICATION_V18_TIMEOUT_MS || "15000")),
);
const REVIEW_DAYS = Math.max(
  7,
  Number(process.env.REVERIFY_DAYS || "30"),
);
const BATCH_SIZE = Math.max(
  1,
  Math.min(100, Number(process.env.VERIFICATION_V18_BATCH_SIZE || "40")),
);
...
function extractTag(html: string, tag: string) {
  ...
  return m ? normalizeText(m[1]).slice(0, 500) : "";
}

const GIFT_RE =
  /\b(?:e[- ]?gift\s*card|gift\s*card|giftcard|gift\s*voucher|voucher|δωροκάρτα|δωροκαρτα|δωροεπιταγ(?:ή|η)|δωροεπιταγές|δωροεπιταγες)\b/i;
const PURCHASE_RE =
  /(?:€|\$|£|\bEUR\b|\bUSD\b|\bGBP\b|\bbuy\b|\bpurchase\b|\bamount\b|\bvalue\b|\bcart\b|add to cart|checkout|αγορά|αγορα|καλάθι|καλαθι|ποσό|ποσο|αξία|αξια)/i;
const BAD_ROLE_RE =
...
async function fetchEvidence(url: string) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        "user-agent":
          "Mozilla/5.0 (compatible; DorokartesVerification/18; +https://dorokartes.gr)",
    ...
    const html = /html|xhtml/i.test(contentType)
      ? await response.text()
      : "";
    const text = normalizeText(html).slice(0, 120_000);
    return {
      ok: response.ok,
...
type Decision = {
  ...
  evidenceGiftKeyword: boolean;
  evidencePurchaseSignal: boolean;
  evidenceBadRole: boolean;
};
...
function decide(target: Target, ev: Awaited<ReturnType<typeof fetchEvidence>>): Decision {
  ...
  const basis = `${ev.title} ${ev.h1} ${ev.text.slice(0, 50_000)}`;
  const urlBasis = (() => {
    try {
      const u = new URL(ev.finalUrl);
  ...
  const giftKeyword =
    GIFT_RE.test(ev.title) ||
    GIFT_RE.test(ev.h1) ||
    GIFT_RE.test(urlBasis) ||
    GIFT_RE.test(ev.text.slice(0, 50_000));
  const purchaseSignal = PURCHASE_RE.test(basis);
  const badRole =
    BAD_ROLE_RE.test(urlBasis) &&
    !GIFT_RE.test(ev.h1) &&
    !GIFT_RE.test(ev.title);
  ...
  const common = {
    ...
    merchantDomain,
    finalDomain,
    evidenceGiftKeyword: giftKeyword,
    evidencePurchaseSignal: purchaseSignal,
    evidenceBadRole: badRole,
  };
  ...
  if (cardDomain !== merchantDomain) {
    return { bucket: "REVIEW", reason: "OFFICIAL_URL_MERCHANT_DOMAIN_MISMATCH", ...common };
  }
  if (finalDomain !== merchantDomain) {
    return { bucket: "REVIEW", reason: "REDIRECT_DOMAIN_MISMATCH", ...common };
  }
  if (badRole) {
    return { bucket: "REVIEW", reason: "TERMS_PRIVACY_GIFTSET_OR_SUPPORT_PAGE", ...common };
  }
  if (!giftKeyword) {
    return { bucket: "REVIEW", reason: "NO_GIFT_CARD_SEMANTIC_EVIDENCE", ...common };
  }
  if (!purchaseSignal) {
    return { bucket: "REVIEW", reason: "NO_PURCHASE_OR_VALUE_SIGNAL", ...common };
  }

  return {
    bucket: "AUTO_SAFE",
    reason: "HTTP_OK_SAME_DOMAIN_GIFT_CARD_AND_PURCHASE_EVIDENCE",
    ...common,
  };
}
...
function writeCsv(plan: any) {
  ...
  const header = [
    "bucket","reason","merchantName","expectedTitle","expectedOfficialUrl",
    "finalUrl","httpStatus","pageTitle","h1","evidenceGiftKeyword",
    "evidencePurchaseSignal","evidenceBadRole"
  ];
...
async function preview() {
  ...
  console.log("Dorokartes Verification Enrichment v18 — PREVIEW");
...
async function applyPlan() {
  ...
  if (saved.planId !== REQUESTED_PLAN_ID) throw new Error("Plan ID mismatch.");

  // Re-fetch only AUTO_SAFE URLs before writing. Any drift aborts the whole apply.
  ...
  const drift = freshChecks.find(
    (x, i) =>
      x.bucket !== "AUTO_SAFE" ||
      x.finalDomain !== saved.safe[i].finalDomain ||
      x.cardDomain !== saved.safe[i].cardDomain,
  );
  if (drift) {
    throw new Error(
      `Live evidence changed or became unsafe for ${drift.merchantName} (${drift.cardId}). Re-run preview.`,
    );
  }

  const batches = chunk(saved.safe as Decision[], BATCH_SIZE);
  let updatedCards = 0;
    ...
    const result = await prisma.$transaction(
      async (tx) => {
        ...
        for (const row of batches[bi]) {
          ...
          if (!card) throw new Error(`Precondition failed for ${row.cardId}`);
          ...
          if (!source) {
            await tx.sourceRecord.create({
              data: {
                sourceType: SourceType.OFFICIAL,
                sourceName: "Verification Enrichment v18",
          ...
          if (!existingEvent) {
            await tx.verificationEvent.create({
              data: {
                ...
                url: row.expectedOfficialUrl,
                notes:
                  `Verification Enrichment v18: HTTP ${row.httpStatus}; same merchant domain; ` +
                  `gift-card semantic evidence + purchase/value signal.`,
                checkedAt: new Date(),
              },
            });
            e++;
...
async function postAudit() {
  ...
  const missingCardSource = await prisma.giftCard.count({
    where: {
      status: "ACTIVE",
      sources: {
        none: { sourceType: SourceType.OFFICIAL, active: true },
      },
    },
  });
  ...
  console.log("Dorokartes Verification Enrichment v18 — POST-AUDIT");
