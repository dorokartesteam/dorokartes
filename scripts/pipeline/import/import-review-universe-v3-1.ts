...
const CSV = path.join(ROOT, "data", "discovery", "review-universe-v3", "auto-safe-greece.csv");
const OUT_DIR = path.join(ROOT, "data", "discovery", "review-universe-v3");
const PLAN_CSV = path.join(OUT_DIR, "import-plan-v1.1.csv");
const MANUAL_CSV = path.join(OUT_DIR, "import-manual-v1.1.csv");
...
function cleanTracking(raw: string) {
  try {
    ...
    for (const k of [...u.searchParams.keys()]) {
      if (/^(srsltid|utm_|gclid|fbclid)/i.test(k)) u.searchParams.delete(k);
    }
    return u.toString();
  } catch {
    return raw;
...
function suspiciousRouteOnly(raw: string) {
  try {
    const u = new URL(raw);
    return /index\.php$/i.test(u.pathname) && (u.search === "?route" || u.searchParams.get("route") === "");
  } catch {
    return true;
  }
}
...
function cleanMerchantName(raw: string, domain: string) {
  ...
  // Never use generic gift-card phrase as merchant name.
  const generic = /^(gift\s*card|giftcard|δωροκάρτα|δωροκάρτες|δωροεπιταγή|αγοράστε μία|ηλεκτρονική|αξίας\s+\d+|50\s*€|gift card 2)$/i;
  if (generic.test(s)) s = "";

  s = s
    .replace(/\s+[|–—]\s+.*$/, "")
    .replace(/^(Δωροκάρτα|Gift Card)\s+/i, "")
    .trim();
...
async function main() {
  ...
  const [discoveries, merchants] = await Promise.all([
    prisma.discoveryItem.findMany({ select: { possibleOfficialUrl: true, sourceUrl: true } }),
  ...
  let hardExcluded = 0, skippedKnown = 0, invalid = 0, duplicateCsv = 0;
  ...
  for (const row of rows) {
    ...
    const out = {
      domain,
      merchantName,
      title: String(row.title || `${merchantName} Gift Card`).slice(0, 220),
    ...
    if (FORCE_MANUAL.has(domain)) {
      out.reason = "explicit_manual_review";
      manual.push(out);
      continue;
    }

    if (suspiciousRouteOnly(url)) {
      out.reason = "truncated_or_weak_opencart_route_url";
      manual.push(out);
      continue;
    }

    eligible.push({ ...out, action: "QUEUE_NEW" });
  }

  const headers = ["domain","merchantName","title","possibleOfficialUrl","score","occurrences","reason","action"];
  writeCsv(PLAN_CSV, eligible, headers);
  writeCsv(MANUAL_CSV, manual, headers);

  console.log("Dorokartes Review Universe v3.1 Importer");
  console.log("========================================");
    ...
    try {
      await prisma.discoveryItem.create({
        data: {
          ...
          sourceName: "Review Universe v3.1",
