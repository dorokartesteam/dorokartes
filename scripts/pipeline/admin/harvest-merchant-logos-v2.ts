...
const PLAN_JSON = path.join(REPORT_DIR, "merchant-logo-harvest-v2-plan.json");
const PLAN_CSV = path.join(REPORT_DIR, "merchant-logo-harvest-v2-plan.csv");
const APPLY_JSON = path.join(REPORT_DIR, "merchant-logo-harvest-v2-apply.json");
const POST_AUDIT_JSON = path.join(REPORT_DIR, "merchant-logo-harvest-v2-post-audit.json");
...
async function loadTargetsByIds(ids?: string[]) {
  ...
  return merchants.map(({ giftCards, ...merchant }) => ({
    ...merchant,
...
async function applyPlan(plan: Plan) {
  ...
  try {
        ...
        for (const action of plan.actions) {
          ...
          await tx.mediaAsset.create({
            data: {
              ...
              altText: `Λογότυπο ${action.merchantName}`,
    ...
    fs.writeFileSync(
      path.join(REPORT_DIR, `merchant-logo-harvest-v2-apply-${plan.planId.slice(0, 16)}.json`),
...
async function postAudit(plan: Plan) {
  ...
  fs.writeFileSync(
    path.join(REPORT_DIR, `merchant-logo-harvest-v2-post-audit-${plan.planId.slice(0, 16)}.json`),
    `${JSON.stringify(report, null, 2)}\n`,
    "utf8",
  );
...
async function main() {
  ...
  if (APPLY) {
    ...
    console.log("Dorokartes Merchant Logo Harvest v2 — APPLY");
  ...
  if (POST_AUDIT) {
    ...
    console.log("Dorokartes Merchant Logo Harvest v2 — POST-AUDIT");
  ...
  console.log("Dorokartes Merchant Logo Harvest v2 — PREVIEW");
  ...
  const immutablePlanJson = path.join(
    REPORT_DIR,
    `merchant-logo-harvest-v2-plan-${plan.planId.slice(0, 16)}.json`,
  );
  const immutablePlanCsv = path.join(
    REPORT_DIR,
    `merchant-logo-harvest-v2-plan-${plan.planId.slice(0, 16)}.csv`,
  );
