...
async function checkCandidate(url: string) {
  ...
  try {
    const r = await fetch(url, {
      ...
      headers: {
        "user-agent":
          "Mozilla/5.0 (compatible; DorokartesCanonicalAudit/1.1; +https://dorokartes.gr)",
    ...
    if (!ct.includes("text/html")) {
      return { ok: false, status: r.status, score: 0 };
    }

    const html = (await r.text()).toLowerCase();
    let score = 0;

    if (/gift\s*card|giftcard|voucher|δωροκάρτ|δωροκαρτ|δωροεπιταγ/.test(html)) {
      score += 40;
    }
...
async function main() {
  ...
  for (const card of targets) {
    ...
    const shouldChangeMerchant =
      proposedMerchant.toLowerCase() !== card.merchant.name.toLowerCase();
    ...
    for (const variant of card.variants) {
      console.log(
        `  VARIANT | ${variant.name} | type=${variant.type} | custom=${variant.customValueAllowed} | active=${variant.active}`
      );
    }
  ...
  const outFile = path.join(outDir, "canonical-giftcard-audit-v4-1.json");
