...
fs.copyFileSync(file, `${file}.backup-before-v3.1`);
...
function firstExistingInput() {
  ...
  for (const x of inspected) {
    console.log(
      `  rows=${x.rows.toString().padStart(4)} score=${x.score.toString().padStart(5)}  ${path.basename(x.file)}`
    );
  }
  console.log("");

  // We need the listing/product export, not the 63-row merchant/seller export.
...
// Change version banner so user can confirm patch loaded.
s = s.replace(
  'Dorokartes BestPrice Inventory + Issuer Extractor v3',
  'Dorokartes BestPrice Inventory + Issuer Extractor v3.1'
);

fs.writeFileSync(file, s, "utf8");

console.log("Patched BestPrice issuer extractor to v3.1");
console.log("- auto-detects the product/listing CSV by row count + headers");
...
console.log("- backup created:", `${file}.backup-before-v3.1`);
