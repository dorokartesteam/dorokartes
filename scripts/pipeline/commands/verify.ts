...
} from "../core/verification-guardrails";
import { classifyWithLlm, PROMPT_VERSION, VERIFICATION_TEMPERATURE, REASONING_EFFORT } from "../core/llm-classifier";
import {
  VERIFICATION_MODEL,
...
function hasStrongGiftCardProgramEvidence(input: {
...
async function main() {
  ...
  for (const item of items) {
    ...
    if (cached) {
      ...
      const cachedProgramEvidence = hasStrongGiftCardProgramEvidence({
    ...
    try {
      ...
      const programEvidence = hasStrongGiftCardProgramEvidence({
