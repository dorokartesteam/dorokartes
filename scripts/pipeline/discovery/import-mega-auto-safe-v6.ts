...
// One clearly suspicious result from the reviewed AUTO_SAFE sample.
// Keep it out of auto-import and leave it for manual review.
const MANUAL_EXCLUDE = new Set([
  "workwide.gr",
]);

async function main() {
  console.log("Dorokartes Import MEGA Auto-Safe v6");
  console.log("===================================");
  ...
  for (const row of plan) {
    await prisma.discoveryItem.create({
      data: {
        ...
        sourceName: "Google Serper MEGA Harvest v6 AUTO_SAFE",
